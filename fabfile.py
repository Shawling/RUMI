...
env.sudo_user = 'root'
# 服务器地址，可以有多个，依次部署:
env.hosts = ['120.78.228.243']

# 服务器MySQL用户名和口令:
db_user = 'root'
db_password = 'Admin123'

_TAR_FILE = 'dist-rumi.tar.gz'

_REMOTE_TMP_TAR = '/tmp/%s' % _TAR_FILE

_REMOTE_BASE_DIR = '/srv/rumi'
...
def backup():
    ...
    '''
    dt = _now()
    f = 'backup-%s-%s.sql' % (pro_name, dt)
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick %s > %s'
            % (db_user, db_password, pro_name, f))
...
def deploy():
    ...
    with cd(_REMOTE_BASE_DIR):
        ...
        sudo('chown %s:%s www' % (env.user, env.user))
        sudo('chown -R %s:%s %s' % (env.user, env.user, newdir))
    # 安装所需的库
    with cd('%s/www' % _REMOTE_BASE_DIR):
        sudo('pip3 install -r requirements.txt')
    # 重启Python服务和nginx服务器
    with settings(warn_only=True):
        sudo('supervisorctl stop %s' % pro_name)
        sudo('supervisorctl start %s' % pro_name)
        sudo('nginx -s reload')
...
def restore2local():
    ...
    sqls = [
        'drop database if exists STAMS;', 'create database STAMS;',
        'grant select, insert, update, delete on STAMS.* to \'%s\'@\'localhost\' identified by \'%s\';'
        % (db_user, db_password)
    ]
    for sql in sqls:
    ...
    local(r'mysql -uroot -p%s stams < backup/%s' % (p, restore_file[:-7]))
    with lcd(backup_dir):
